...
// Special Tab for the Leaderboard
          ...
          <div className="flex space-x-8 mt-2 overflow-x-auto no-scrollbar">
            ...
            <button
                ...
                className={`pb-4 text-sm font-medium tracking-wide transition-all border-b-2 whitespace-nowrap px-2 flex items-center gap-2 ${
                  activeTab === TAB_CONTRIBUTORS ? 'border-yellow-500 text-yellow-400' : 'border-transparent text-zinc-500 hover:text-yellow-200'
                }`}
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {contributors.map((contributor, index) => (
                        <div key={index} className="bg-zinc-900/50 border border-white/5 p-6 rounded-2xl flex items-center gap-4 hover:bg-zinc-800/50 transition-colors">
                            <div className="relative">
                                <img 
                                    src={contributor.photo || `https://ui-avatars.com/api/?name=${contributor.name}&background=random`} 
                                    alt={contributor.name}
                                    className="w-16 h-16 rounded-full border-2 border-zinc-700 object-cover" 
                                />
                                {index < 3 && (
                                    <div className={`absolute -top-2 -right-2 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold border-2 border-zinc-900
                    ...
                    {contributors.length === 0 && (
                        <div className="col-span-full text-center py-20 text-zinc-500">
                            No contributions recorded yet. Be the first!
                        </div>
                    )}
                </div>
            </div>
        ...
        {activeTab !== TAB_CONTRIBUTORS && (
            <>
                ...
                <div className="text-center py-16 border border-dashed border-zinc-800 rounded-2xl">
                    ...
                    <button onClick={onAddClick} className="px-6 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-full font-medium transition-colors">Add the First {activeTab}</button>
                </div>
                )}
