...
import { calculateCineRank } from '../cineRank'; // Make sure Step 2 is done!
...
interface MediaCardProps {
  ...
  onRate?: (item: MediaItem, rating: number) => void; // New Prop
  isAdmin?: boolean;
...
export const MediaCard: React.FC<MediaCardProps> = ({ 
  ...
  const handleImageError = () => {
    if (imgSrc !== generatedUrl) setImgSrc(generatedUrl);
  };
  ...
  const ratingCount = Object.keys(item.starRatings || {}).length;
  
  // Total people who interacted
  const totalInteractions = likesCount + dislikesCount + ratingCount;
  ...
  // Get user's current star rating (if any)
  const userRating = (currentUserEmail && item.starRatings) ? item.starRatings[currentUserEmail] || 0 : 0;

  // Dynamic Color for Score
  const getScoreColor = (score: number) => {
    if (totalInteractions === 0) return 'text-zinc-500';
    ...
    >
      ...
      <div className="p-3 md:p-5 flex flex-col justify-between flex-grow min-w-0">
        <div>
          ...
          {/* --- NEW: STAR RATING INTERFACE --- */}
          <div className="flex items-center gap-0.5 mb-2" onMouseLeave={() => setHoverRating(0)}>
            {[1, 2, 3, 4, 5].map((star) => (
                <button
                    key={star}
                    onClick={(e) => { e.stopPropagation(); onRate?.(item, star); }}
            ...
            <span className="text-[10px] text-zinc-500 ml-2 font-mono">{ratingCount > 0 ? `(${ratingCount})` : ''}</span>
          </div>
          {/* --------------------------------- */}
        ...
        <div className="flex items-end justify-between mt-2 md:mt-4 pt-2 md:pt-4 border-t border-white/5">
          ...
          {/* --- CINERANK SCORE BADGE --- */}
          <div className="flex items-center gap-3">
            <div className="flex flex-col">
              <span className="text-[8px] md:text-[10px] text-zinc-500 uppercase tracking-widest font-bold">CineRank</span>
              <div className="flex items-baseline gap-1.5">
                <span className={`text-xl md:text-2xl font-black ${getScoreColor(cineRankScore)}`}>
                  {totalInteractions === 0 ? 'NR' : cineRankScore}
                </span>
              </div>
            </div>
          </div>
